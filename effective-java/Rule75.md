...
기본 직렬화 형식을 사용하건, 말건 `transient` 로 선언되지 않은 모든 객체 필드는 `defaultWriteObject` 메서드가 호출될 때 직렬화 된다. 따라서 `transient` 로 선언할 수 있는 모든 객체 필드는, 그렇게 선언해야 한다. 

- 캐시된 해시값 처럼, *primary data field* 에서 계산 될 수 있는 값을 담는 중복 필드에도 `transient` 를 붙어야 한다.
- JVM 이 실행될 때마다 달라지는 값을 갖는 필드도 마찬가지다. 예를 들어 네이티브 자료구조에 대한 포인터를 나타내는 `long` 필드

> 객체의 논리적 상태를 구성하는 값이라는 확신이 들기 전에는 `non-transient` 필드로 만들어야 겠다는 결정을 내리지 마라. 

<br/>

**사용자 직렬화 형식을 사용할 경우**, `transient` 레이블이 붙은 필드들은 역직렬화 되었을 때 기본값으로 초기화 된다. (객체 참조 필드면 `null`, 불리언은 `false`). 
`transient` 필드에 이런 값이 할당되면 안되는 경우 `defaultReadObject` 를 호출하는 `readObject` 내에서 `transient` 필드의 값을 복구해야 한다.
  
<br/> 

기본 직렬화 형식 사용 여부에 상관없이, **객체를 직렬화 할 때는 객체의 상태 전부를 읽는 메서드에 적용할 동기화 수단을 반드시 적용해야 한다. 예를 들어 모든 메서드를 동기화 해서 스레드 안전성을 달성하는 객체의 경우는 
아래와 같은 `writeObject` 함수를 작성해야 한다.
...
이와는 달리 `writeObject` 내에서 동기화를 하고싶다면, 클래스의 다른 부분에서 사용하는 것과 동일한 락 순서를 따르도록 만들어야한다. 

## 커스텀이든, 기본 직렬화든, 직렬화 가능 클래스를 만들 경우에는 Serial Version UID 를 명시적으로 선언해야 한다.
 
 - Serial Version UID 때문에 생길 수 있는 문제를 방지할 수 있다.
 - 성능이 조금 개선된다
 
## 요약

직렬화 가능한 클래스를 만들기로 했다면, 직렬화 형식에 대해서도 심각하게 고민해야 한다는 것이다. 기본 직렬화 형식은 그 형태가 **논리적 상태에 부합할 때만** 사용해야 한다. 
그렇지 않다면 객체의 논리적 상태를 적절히 표현하는 사용자 정의 직렬화 형식을 만들라. 

직렬화 형식에 포함시킨 필드는 나중에 제거할 수 없다. 직렬화 호화성을 보장하기 위해서다. 직렬화 형식을 잘못 설계하면, 클래스의 복잡성과 성능 측면에 돌이킬 수 없는 악영향을 끼치게 된다.
